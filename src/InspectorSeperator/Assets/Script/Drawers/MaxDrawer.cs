...
public class MaxDrawer : PropertyDrawer {
	...
	MaxAttribute _attribute { get { return ((MaxAttribute)attribute); } }
	...
	public override void OnGUI (Rect position, SerializedProperty prop, GUIContent label) {
		...
		float constraintValue = _attribute.maxValue;
		...
		if (proptype == (typeof(Vector2).Name+"f")) {
			...
			val = val > constraintValue ? Mathf.FloorToInt(constraintValue) : val;
			...
			EditorGUI.LabelField(position, label, new GUIContent("Use Max with Vectors or numbers."));
		}
